namespace FestivalManager.Entities
{
    ...
    public class Stage
	{
        ...
        public void AddPerformer(Performer performer) //if null->exception
        {
            this.ValidateNullValue(performer, nameof(performer), CanNotBeNullMessage);

            if (performer.Age < 18) //if age < 18 -> exception
            {
                throw new ArgumentException("You can only add performers that are at least 18.");
            }

            this.performers.Add(performer); //if normal performer->exception
        }
