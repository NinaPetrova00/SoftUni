namespace Theatre.DataProcessor
{
    ...
    public class Deserializer
    {
        ...
        public static string ImportPlays(TheatreContext context, string xmlString)
        {
            ...
            ImportPlayDto[] playsDtos = (ImportPlayDto[])xmlSerializer.Deserialize(stringReader);
            ...
            foreach (var playDto in playsDtos)
            {
                ...
                if (duration.Hours < GlobalConstants.Play_Duartion_Min_Length)
                {
                    sb.AppendLine(ErrorMessage);
                    continue;
                }
        ...
        public static string ImportCasts(TheatreContext context, string xmlString)
        {
            ...
            ImportCastDto[] castsDtos = (ImportCastDto[])xmlSerializer.Deserialize(stringReader);

            HashSet<Cast> validCasts = new HashSet<Cast>();
            foreach (ImportCastDto castDto in castsDtos)
            {
                if (!IsValid(castDto))
                {
                    sb.AppendLine(ErrorMessage);
